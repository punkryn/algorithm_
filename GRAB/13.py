...
def solution(n, k, paths):
    ...
    while q:
        cost, mile, now = heapq.heappop(q)

        if distance[now] < cost:
            continue

        for path in graph[now]:
            nxt, _cost, _mile = path
            # print(nxt)
            dist = cost + _cost
            m = mile + _mile
            if distance[nxt] >= dist:
                distance[nxt] = dist
                # result[nxt].append((dist, now, m))
                if mileage[nxt] < m:
                    mileage[nxt] = m
                heapq.heappush(q, (dist, m, nxt))

    # print(result)
    # print(distance, mileage)

    answer.append(distance[k])
    answer.append(mileage[k])

    return answer

n = 5
k = 4
paths = [[1 ,5 ,1 ,1] ,[1 ,2 ,4 ,3] ,[1 ,3 ,3 ,2] ,[2 ,5 ,2 ,1] ,[2 ,4 ,2 ,3] ,[3 ,4 ,2 ,2]]
print(solution(n, k, paths))
...
위 그림에서 원에 적힌 숫자는 공항 번호를 나타내며, 각 선분은 항로를 나타냅니다. 또, 선분에 적힌 숫자는 "이동 시간 / 적립 마일리지"를 나타냅니다. 예를 들어 1번 공항에서 2번 공항으로 이동할 때는 4시간이 걸리며, 3 마일리지가 적립됩니다.
