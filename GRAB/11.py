...
위 그림에서 동그라미 안에 적힌 숫자는 노드 번호, 간선에 적힌 숫자는 간선 번호를 나타냅니다.
위 예시와 같이 붉은색 간선(2번, 5번)을 제거하면 세 부분 트리로 나뉘며, 각 부분 트리에 속한 노드 개수는 3개로 모두 같습니다.
이때, 세 부분으로 분리된 부분 트리는 노드 수만 같으면 되며, 트리의 모양까지 같을 필요는 없습니다.

트리의 노드 개수 n, 트리의 간선 정보가 담긴 배열 edges가 매개변수로 주어질 때,
노드 수가 같은 부분 트리 세 개로 분리하기 위해 제거해야 하는 간선의 번호를 순서대로 배열에 담아 return 하도록 solution 함수를 완성해주세요.
